...
`demos/node-client` is a dependency-light node bridge and browser page that
watches a book unpack live: the upload bar, then `info`, then each chapter as
its entry comes off the archive. It serves under a configurable `UI_BASE`
(`/ui/epub` in the demo shell) on port 8086:
...
The sample books under `demos/sample-data` are generated by
`npm run samples`, never committed.
...
| `demos/node-client/` | the web viewer: a node:http-to-gRPC bridge plus its page |
| `demos/sample-data/` | the viewer's books, generated by `npm run samples`; nothing binary is committed here either |
